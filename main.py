            ...
            if result.returncode == 0:
                ...
                if "model not found" in stderr.lower():
                    ...
                    try:
                        ...
                        model_files = [f for f in os.listdir(cache_dir) if f.startswith(self.model_name)]
    ...
    def run(self):
        ...
        try:
            ...
            if self.set_system_env_variable("OLLAMA_MODELS", models_dir):
                ...
                self.log_signal.emit("ВАЖНО: Требуется перезагрузка компьютера для применения изменений")
            else:
                self.log_signal.emit("ВНИМАНИЕ: Не удалось установить системную переменную OLLAMA_MODELS")
                self.log_signal.emit("Установка продолжится, но может потребоваться ручная настройка")
            ...
            if self.is_cancelled:
                ...
                if custom_files:
                    ...
                    self.log_signal.emit("Проверьте, что Ollama имеет права на запись в указанную директорию")
...
class OllamaAgent(QMainWindow):
    ...
    def start_install(self):
        model_name = self.model_name_input.text().strip() or self.recommended_combo.currentText().split(" (")[0]
        if not model_name:
    ...
    def install_finished(self, success):
        ...
        if success:
            self.update_model_list()
            self.log(f"Модель {self.model_name_input.text()} установлена в {self.install_dir}/models/{self.model_name_input.text()}!")
        else:
